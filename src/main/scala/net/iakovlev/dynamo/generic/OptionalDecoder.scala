...
object SingleFieldDecoder {
  ...
  implicit def mapAsClassDecoder[A](implicit dm: OptionalDecoder[A],
                                    t: Typeable[A]) =
...
trait OptionalDecoder[A] {
  def decode(attributes: Map[String, AttributeValue]): Option[A]
}

object OptionalDecoder {

  implicit def hNilDecoder = new OptionalDecoder[HNil] {
    override def decode(
  ...
  implicit def hConsDecoder[K <: Symbol, H, T <: HList](
      ...
      dt: OptionalDecoder[T],
      t: Typeable[H]) = new OptionalDecoder[FieldType[K, H] :: T] {
  ...
  implicit def caseClassDecoder[A, R](implicit lg: LabelledGeneric.Aux[A, R],
                                      dr: OptionalDecoder[R],
                                      t: Typeable[A]) =
    new OptionalDecoder[A] {
  ...
  def apply[A](attributes: Map[String, AttributeValue])(
      implicit da: OptionalDecoder[A]): Option[A] = {
